...
export const metadata = {
  title: "Tennessee Titans 2026 Roster | Touchdown Tennessee",
  description: "Full roster for the Tennessee Titans NFL team — players, positions, jersey numbers.",
};
...
export default async function TitansRosterPage() {
  let groups: RosterGroup[] = [];
  let teamName = "Tennessee Titans";
  ...
  try {
    ...
    console.error("Titans roster fetch error:", e);
  }
  ...
  return (
    <main style={{ fontFamily: "Georgia, serif", background: "#fff", color: "#1A1208", minHeight: "100vh" }}>
      ...
      <div style={{ width: "100%", maxHeight: 300, overflow: "hidden", borderBottom: "3px solid #4B92DB" }}>
        <Image src="/titans-stadium-charge.png" alt="Tennessee Titans" width={1800} height={600} style={{ width: "100%", height: "auto", display: "block", objectFit: "cover", objectPosition: "center 30%" }} priority />
      </div>

      {/* HEADER */}
      <div style={{ maxWidth: 1080, margin: "40px auto 0", padding: "0 40px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
          <span style={{ border: "1.5px solid #4B92DB", color: "#4B92DB", fontSize: 9, fontWeight: 700, letterSpacing: "0.2em", padding: "3px 8px", textTransform: "uppercase" }}>Titans Desk</span>
        ...
        {sorted.map((group) => (
          <div key={group.position} style={{ marginBottom: 48 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 16 }}>
              <h3 style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.22em", textTransform: "uppercase", margin: 0 }}>{group.position}</h3>
              <div style={{ flex: 1, height: 1, background: "#4B92DB" }} />
            </div>
                ...
                <div key={player.id} style={{ display: "grid", gridTemplateColumns: "48px 48px 1fr 80px 80px 80px 100px 140px", padding: "10px 16px", borderTop: "1px solid #D4CEC7", background: i % 2 === 0 ? "#fff" : "#FAFAF8", gap: 8, alignItems: "center" }}>
                  ...
                  <div style={{ width: 36, height: 36, borderRadius: "50%", overflow: "hidden", background: "#F0EDE8", flexShrink: 0 }}>
                    {player.headshot?.href ? (
                      ...
                      <div style={{ width: 36, height: 36, background: "#1A1208", display: "flex", alignItems: "center", justifyContent: "center", color: "#4B92DB", fontSize: 11, fontWeight: 700 }}>
