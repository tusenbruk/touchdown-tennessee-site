...
import { trackViewItem } from "@/app/components/analytics";
    ...
    fetch(`/api/merch/product?id=${id}`)
      ...
      .then((data) => {
        ...
        if (data?.id) {
          ...
          trackViewItem({ item_id: String(data.id), item_name: data.name, price: first ? parseFloat(first.retail_price) : undefined });
        }
      })
